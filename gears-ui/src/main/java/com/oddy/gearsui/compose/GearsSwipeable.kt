...
open class GearsSwipeableState<T>(
    initialValue: T,
    internal val animationSpec: AnimationSpec<Float> = SwipeableDefaults.AnimationSpec,
) {
    internal open fun confirmStateChange(newValue: T): Boolean {
        return true
    }
     ...
     * [gearsSwipeable] is currently settled. If a swipe or animation is in progress, this corresponds
     * the last anchor at which the [gearsSwipeable] was settled before the swipe or animation started.
     */
    ...
    /**
     * The current position (in pixels) of the [gearsSwipeable].
     *
    ...
    /**
     * The amount by which the [gearsSwipeable] has been swiped past its bounds.
     */
     ...
     *
     * If a swipe is in progress, this is the value that the [gearsSwipeable] would animate to if the
    ...
    /**
     * Information about the ongoing swipe or animation, if any. See [GearsSwipeProgress] for details.
     *
     * If no swipe or animation is in progress, this returns `SwipeProgress(value, value, 1f)`.
     */
    @ExperimentalMaterialApi
    val progress: GearsSwipeProgress<T>
        get() {
            ...
            return GearsSwipeProgress(from, to, fraction)
        }

    /**
     * The direction in which the [gearsSwipeable] is moving, relative to the current [currentValue].
     *
     ...
     * [velocity]. Fling with settling [gearsSwipeable] will always consume all the velocity provided
     * since it will settle at the anchor.
     *
     * In general cases, [gearsSwipeable] flings by itself when being swiped. This method is to be
     * used for nested scroll logic that wraps the [gearsSwipeable]. In nested scroll developer may
    ...
    /**
     * Force [gearsSwipeable] to consume drag delta provided from outside of the regular [gearsSwipeable]
     * gesture flow.
     *
     ...
     *
     * In general cases, [gearsSwipeable] drags by itself when being swiped. This method is to be
     * used for nested scroll logic that wraps the [gearsSwipeable]. In nested scroll developer may
...
/**
 * Collects information about the ongoing swipe or animation in [gearsSwipeable].
 *
...
@ExperimentalMaterialApi
internal fun <T> Modifier.gearsSwipeable(
    state: GearsSwipeableState<T>,
    anchors: Map<Float, T>,
...
@ExperimentalMaterialApi
internal val <T> GearsSwipeableState<T>.PreUpPostDownNestedScrollConnection: NestedScrollConnection
